...
const Post = ({ buyNow, addToCart, product, variants }) => {
  ...
  // console.log(addToCart, removeFromCart, cart, totalamount, clearCart)
      ...
      <section className="text-gray-600 body-font overflow-hidden">
        ...
        <div className="container px-5 py-24 mx-auto">
          <div className="lg:w-4/5 mx-auto flex flex-wrap">
            <img alt="ecommerce" className="lg:w-1/2 w-full lg:h-auto] px-20 object-cover object-top rounded" src="/T-shirt1.jpg" />
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              <h2 className="text-sm title-font text-gray-500 tracking-widest">CODESWEAR.COM</h2>
              <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{product.title}(Size: {product.size}/Color: {product.color})</h1>
            ...
            <a className="text-gray-500">
              ...
              <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                <div className="flex">
                  ...
                  {Object.keys(variants).includes('white') && Object.keys(variants['white']).includes(size) && <  button onClick={() => { refreshVariant(size, "white") }} className={`border-2  ml-1 bg-white-700 rounded-full w-6 h-6 focus:outline-none ${color == 'white' ? "border-black" : "border-gray-300"}  `}   ></button>}
              ...
              <div className="flex">
                ...
                <button className="flex ml-8 text-white bg-pink-500 border-0 py-2 px-4 focus:outline-none hover:bg-pink-600 rounded" onClick={() => { buyNow(slug, 1, product.title, product.price, product.size, product.color) }}>Buy Now</button>
                <button className="flex ml-2 text-white bg-pink-500 border-0 py-2 px-4 focus:outline-none hover:bg-pink-600 rounded" onClick={() => { addToCart(slug, 1, product.title, product.price, product.size, product.color) }}>Add to Cart</button>
              ...
              <div className="flex mt-5">
                <input onChange={handlechange} type="text" className='border-2 border-pink-200 rounded-md' />
                <button onClick={handlecheck} className="flex ml-2 text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded">Check</button>
              </div>
              {!accessability && (accessability != null) && <div>sorry! the area hava no serviceability </div>}
              {accessability && (accessability != null) && <div>ya! you can order</div>}
            </div>
          </div>
        </div>
      </section>
